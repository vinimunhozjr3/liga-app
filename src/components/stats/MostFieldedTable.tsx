...
export function MostFieldedTable({ rows }: { rows: PlayerStats[] }) {
  if (rows.length === 0) {
    return <EmptyState title="Nenhuma escalação importada ainda" />
  }
    ...
    <ol className="divide-y divide-slate-100 rounded-xl border border-slate-200">
      {rows.slice(0, 20).map((row, i) => (
        <li key={`${row.team_id}-${row.player_id}`} className="flex items-center gap-3 px-4 py-2.5">
          ...
          <span className="text-sm font-bold text-slate-900">{row.appearances}</span>
        </li>
      ))}
    </ol>
  )
