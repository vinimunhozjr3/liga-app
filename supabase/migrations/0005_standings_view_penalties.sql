-- =========================================================
-- 0005_standings_view_penalties.sql
-- =========================================================
-- Recria a view standings incluindo vitorias em penaltis como
-- criterio de desempate: Pontos > Vitorias em penaltis > Saldo
-- de gols > Gols marcados > Nome.
...
aggregated as (
  select
    ...
    count(*) filter (where tm.result = 'E' and tm.penalty_winner_team_id = tm.team_id) as penalty_wins
...
select
  ...
  row_number() over (
    ...
    order by a.points desc, a.penalty_wins desc, a.goal_diff desc, a.goals_for desc, t.name asc
  ) as position,
  a.penalty_wins
from aggregated a
